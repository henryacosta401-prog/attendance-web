def register_employee_routes(app, deps):
    ...
    DEFAULT_BREAK_WINDOW_END = deps["DEFAULT_BREAK_WINDOW_END"]
    DEFAULT_BREAK_WINDOW_START = deps["DEFAULT_BREAK_WINDOW_START"]
    DEFAULT_PAID_LEAVE_DAYS = deps["DEFAULT_PAID_LEAVE_DAYS"]
    DEFAULT_SCHEDULE_DAYS = deps["DEFAULT_SCHEDULE_DAYS"]
    DEFAULT_SHIFT_END = deps["DEFAULT_SHIFT_END"]
    DEFAULT_SHIFT_START = deps["DEFAULT_SHIFT_START"]
    DEFAULT_SICK_LEAVE_DAYS = deps["DEFAULT_SICK_LEAVE_DAYS"]
    IMAGE_EXTENSIONS = deps["IMAGE_EXTENSIONS"]
    Response = deps["Response"]
    SCHEDULE_SPECIAL_RULE_LABELS = deps["SCHEDULE_SPECIAL_RULE_LABELS"]
    SCHEDULE_SPECIAL_RULE_OPTIONS = deps["SCHEDULE_SPECIAL_RULE_OPTIONS"]
    WEEKDAY_OPTIONS = deps["WEEKDAY_OPTIONS"]
    build_future_schedule_change_map = deps["build_future_schedule_change_map"]
    build_schedule_special_rule_label = deps["build_schedule_special_rule_label"]
    create_notification = deps["create_notification"]
    delete_uploaded_file_if_unused = deps["delete_uploaded_file_if_unused"]
    execute_db = deps["execute_db"]
    fetchall = deps["fetchall"]
    fetchone = deps["fetchone"]
    find_employee_identifier_conflict = deps["find_employee_identifier_conflict"]
    flash = deps["flash"]
    generate_code128_svg_markup = deps["generate_code128_svg_markup"]
    generate_password_hash = deps["generate_password_hash"]
    get_future_schedule_changes = deps["get_future_schedule_changes"]
    get_recent_employee_schedule_history = deps["get_recent_employee_schedule_history"]
    get_schedule_day_codes = deps["get_schedule_day_codes"]
    get_schedule_preset = deps["get_schedule_preset"]
    get_schedule_presets = deps["get_schedule_presets"]
    get_schedule_special_dates = deps["get_schedule_special_dates"]
    get_db = deps["get_db"]
    get_schedule_summary = deps["get_schedule_summary"]
    ...
    invalidate_schedule_change_apply_state = deps["invalidate_schedule_change_apply_state"]
    invalidate_schedule_special_rule_cache = deps["invalidate_schedule_special_rule_cache"]
    log_activity = deps["log_activity"]
    login_required = deps["login_required"]
    normalize_schedule_days = deps["normalize_schedule_days"]
    normalize_schedule_special_rule_type = deps["normalize_schedule_special_rule_type"]
    now_dt = deps["now_dt"]
    ...
    parse_shift_end = deps["parse_shift_end"]
    parse_shift_start = deps["parse_shift_start"]
    queue_future_schedule_change = deps["queue_future_schedule_change"]
    record_employee_schedule_history = deps["record_employee_schedule_history"]
    redirect = deps["redirect"]
    render_template = deps["render_template"]
    request = deps["request"]
    resolve_schedule_assignment = deps["resolve_schedule_assignment"]
    save_uploaded_file = deps["save_uploaded_file"]
    schedule_preset_matches_department = deps["schedule_preset_matches_department"]
    secure_filename = deps["secure_filename"]
    session = deps["session"]
    summarize_employee_admin_changes = deps["summarize_employee_admin_changes"]
