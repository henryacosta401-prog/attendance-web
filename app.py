...
from attendance_core.migrations import apply_schema_migrations
from attendance_core.parsing import (
    paginate_items,
    parse_money_value,
...
from attendance_core.visual_helpers import (
    generate_code128_svg_data_uri,
    generate_code128_svg_markup,
    get_avatar_initials,
)
from attendance_core.workbook_services import (
    append_workbook_rows,
    autosize_workbook_sheet,
    build_recovery_pack_workbook,
    configure_recovery_pack_services,
    workbook_to_response,
)
from attendance_core.incident_services import (
    configure_incident_services,
    build_incident_policy_details,
    count_repeated_incidents,
    create_disciplinary_action,
    create_incident,
    find_conflicting_disciplinary_action,
    get_disciplinary_action_by_id,
    get_disciplinary_actions,
    get_employee_error_record_summary,
    get_incident_error_type_options,
    get_incident_policy_action,
    get_overlap_days,
    get_suspension_for_date,
    normalize_incident_error_type,
    sync_incident_policy,
)
from attendance_core.payslip_services import (
    configure_payslip_services,
    attach_payslip_download_requests,
    enrich_payslip_download_request,
    get_payroll_admin_recipients,
    get_payslip_download_request,
    get_payslip_download_request_by_id,
    get_payslip_download_request_map,
    get_payslip_download_request_summary,
    get_recent_payslip_download_requests,
    review_payslip_download_request,
    submit_payslip_download_request,
)
...
from attendance_core.auth_routes import register_auth_routes
...
from attendance_core.admin_profile_routes import register_admin_profile_routes
...
try:
    ...
    cloudinary_uploader_spec = importlib.util.find_spec("cloudinary.uploader") if cloudinary_spec else None
    if cloudinary_spec and cloudinary_uploader_spec:
...
def cursor_has_duplicate_employee_identifier_values(cursor, field_name, postgres=False):
    if field_name not in {"barcode_id", "employee_code"}:
        return False
    trim_expr = f"BTRIM(COALESCE({field_name}, ''))" if postgres else f"TRIM(COALESCE({field_name}, ''))"
    cursor.execute(f"""
        SELECT 1
        FROM users
        WHERE role = 'employee'
...
def ensure_employee_identifier_unique_index_sqlite(cursor, field_name, index_name):
    if cursor_has_duplicate_employee_identifier_values(cursor, field_name, postgres=False):
        return
    try:
        cursor.execute(f"""
            CREATE UNIQUE INDEX IF NOT EXISTS {index_name}
            ON users(TRIM(COALESCE({field_name}, '')))
            WHERE role = 'employee' AND TRIM(COALESCE({field_name}, '')) <> ''
        """)
    except (sqlite3.IntegrityError, sqlite3.OperationalError):
        pass


def ensure_employee_identifier_unique_index_postgres(cursor, field_name, index_name):
    if cursor_has_duplicate_employee_identifier_values(cursor, field_name, postgres=True):
        return
    try:
        cursor.execute("SAVEPOINT employee_identifier_unique_index")
        cursor.execute(f"""
            CREATE UNIQUE INDEX IF NOT EXISTS {index_name}
            ON users ((BTRIM(COALESCE({field_name}, ''))))
            WHERE role = 'employee' AND BTRIM(COALESCE({field_name}, '')) <> ''
        """)
        cursor.execute("RELEASE SAVEPOINT employee_identifier_unique_index")
    except Exception:
        cursor.execute("ROLLBACK TO SAVEPOINT employee_identifier_unique_index")
        cursor.execute("RELEASE SAVEPOINT employee_identifier_unique_index")
...
def init_sqlite_db():
    ...
    apply_schema_migrations(cursor, postgres=False, now_func=now_str)
    ...
    if "employee_code" not in existing_cols_users:
    ...
    cursor.execute("""
        UPDATE users
        SET employee_code = printf('ID-%04d', id)
        WHERE role = 'employee' AND TRIM(COALESCE(employee_code, '')) = ''
    """)
    ensure_employee_identifier_unique_index_sqlite(cursor, "employee_code", "idx_users_employee_code_unique")
    ensure_employee_identifier_unique_index_sqlite(cursor, "barcode_id", "idx_users_employee_barcode_unique")
...
def init_postgres_db():
    ...
    with db.cursor() as cur:
        ...
        cur.execute("""
            UPDATE users
            SET employee_code = 'ID-' || LPAD(CAST(id AS TEXT), 4, '0')
            WHERE role = 'employee' AND BTRIM(COALESCE(employee_code, '')) = ''
        """)
        ensure_employee_identifier_unique_index_postgres(cur, "employee_code", "idx_users_employee_code_unique")
        ensure_employee_identifier_unique_index_postgres(cur, "barcode_id", "idx_users_employee_barcode_unique")
        ...
        apply_schema_migrations(cur, postgres=True, now_func=now_str)
...
configure_recovery_pack_services({
    "fetchall": fetchall,
    "get_backup_recovery_snapshot": get_backup_recovery_snapshot,
...
register_admin_profile_routes(app, {
    ...
    "get_admin_role_preset_meta": get_admin_role_preset_meta,
    "get_company_settings": get_company_settings,
    "get_scanner_account": get_scanner_account,
    "get_user_by_id": get_user_by_id,
    "has_scanner_exit_pin": has_scanner_exit_pin,
    "login_required": login_required,
    "log_activity": log_activity,
    "normalize_admin_role_preset": normalize_admin_role_preset,
    "now_str": now_str,
    ...
    "session": session,
    "sync_admin_role_preset": sync_admin_role_preset,
    "url_for": url_for,
})
...
register_admin_incident_routes(app, {
    ...
    "INCIDENT_DISCIPLINARY_POLICY": INCIDENT_DISCIPLINARY_POLICY,
    "Response": Response,
    "calculate_suspension_end_date": calculate_suspension_end_date,
    "create_disciplinary_action": create_disciplinary_action,
    "create_incident": create_incident,
    "execute_db": execute_db,
    "fetchall": fetchall,
    "fetchone": fetchone,
    "find_conflicting_disciplinary_action": find_conflicting_disciplinary_action,
    "flash": flash,
    "get_department_options": get_department_options,
    "get_disciplinary_action_by_id": get_disciplinary_action_by_id,
    "get_disciplinary_actions": get_disciplinary_actions,
    "get_employee_options": get_employee_options,
    "get_incident_error_type_options": get_incident_error_type_options,
    "get_incident_reports": get_incident_reports,
    "get_user_by_id": get_user_by_id,
    "log_activity": log_activity,
    "login_required": login_required,
    "normalize_incident_error_type": normalize_incident_error_type,
    "now_str": now_str,
    ...
    "request": request,
    "session": session,
    "sync_incident_policy": sync_incident_policy,
    "today_str": today_str,
    "url_for": url_for,
})


register_admin_data_tools_routes(app, {
    "BytesIO": BytesIO,
    "Response": Response,
    "build_recovery_pack_workbook": build_recovery_pack_workbook,
    "build_upload_storage_audit": build_upload_storage_audit,
    ...
    "get_log_cleanup_summary": get_log_cleanup_summary,
    "get_suspicious_attendance_records": get_suspicious_attendance_records,
    "get_user_by_id": get_user_by_id,
    "log_activity": log_activity,
    "login_required": login_required,
    "minutes_to_hm": minutes_to_hm,
    "os": os,
    "perform_go_live_reset": perform_go_live_reset,
    "perform_log_cleanup_for_date_range": perform_log_cleanup_for_date_range,
    "perform_log_retention_cleanup": perform_log_retention_cleanup,
    "record_external_backup_marker": record_external_backup_marker,
    "redirect": redirect,
    "render_template": render_template,
    "request": request,
    "session": session,
    "today_str": today_str,
    "update_attendance_record_by_admin": update_attendance_record_by_admin,
    "url_for": url_for,
    "using_postgres": using_postgres,
    "workbook_to_response": workbook_to_response,
})


register_admin_correction_routes(app, {
    "ABSENT_REQUEST_TYPES": ABSENT_REQUEST_TYPES,
    "APPROVED_SPECIAL_REQUEST_TYPES": APPROVED_SPECIAL_REQUEST_TYPES,
    "DATE_RANGE_REQUEST_TYPES": DATE_RANGE_REQUEST_TYPES,
    "NO_TIMESTAMP_REQUEST_TYPES": NO_TIMESTAMP_REQUEST_TYPES,
    "apply_attendance_correction": apply_attendance_correction,
    "combine_work_date_and_time": combine_work_date_and_time,
    "create_notification": create_notification,
    "describe_request_review_result": describe_request_review_result,
    "execute_db": execute_db,
    "fetchone": fetchone,
    "find_overlapping_date_range_request": find_overlapping_date_range_request,
    "flash": flash,
    "format_request_date_range": format_request_date_range,
    "get_attendance_dates_in_request_range": get_attendance_dates_in_request_range,
    "get_correction_requests": get_correction_requests,
    "get_matching_attendance_context_for_request": get_matching_attendance_context_for_request,
    "log_activity": log_activity,
    "login_required": login_required,
    "normalize_optional_clock_time": normalize_optional_clock_time,
    "now_str": now_str,
    "redirect": redirect,
    "render_template": render_template,
    "request": request,
    "resolve_correction_datetimes": resolve_correction_datetimes,
    "session": session,
    "url_for": url_for,
})
...
register_employee_correction_routes(app, {
    "ABSENT_REQUEST_TYPES": ABSENT_REQUEST_TYPES,
    "ATTENDANCE_REQUEST_TYPES": ATTENDANCE_REQUEST_TYPES,
    "DATE_RANGE_REQUEST_TYPES": DATE_RANGE_REQUEST_TYPES,
    "build_correction_request_summary": build_correction_request_summary,
    "execute_db": execute_db,
    "find_overlapping_date_range_request": find_overlapping_date_range_request,
    "flash": flash,
    "format_request_date_range": format_request_date_range,
    "get_attendance_context": get_attendance_context,
    "get_attendance_dates_in_request_range": get_attendance_dates_in_request_range,
    "get_correction_requests": get_correction_requests,
    "get_leave_balance_summary": get_leave_balance_summary,
    "get_user_by_id": get_user_by_id,
    "log_activity": log_activity,
    "login_required": login_required,
    "normalize_optional_clock_time": normalize_optional_clock_time,
    "normalize_request_date_range": normalize_request_date_range,
    "now_str": now_str,
    "redirect": redirect,
    "render_template": render_template,
    "request": request,
...
register_employee_attendance_routes(app, {
    "DOCUMENT_EXTENSIONS": DOCUMENT_EXTENSIONS,
    "IMAGE_EXTENSIONS": IMAGE_EXTENSIONS,
    "append_attendance_to_google_sheet": append_attendance_to_google_sheet,
    "auto_close_stale_attendance": auto_close_stale_attendance,
    "create_notification": create_notification,
    "describe_tardiness_policy_adjustment": describe_tardiness_policy_adjustment,
    "execute_db": execute_db,
    "flash": flash,
    "get_attendance_by_id": get_attendance_by_id,
    "get_attendance_override_block_message": get_attendance_override_block_message,
    "get_current_attendance": get_current_attendance,
    "get_employee_break_limit": get_employee_break_limit,
    "get_employee_override_status_for_date": get_employee_override_status_for_date,
    "get_manual_attendance_block_message": get_manual_attendance_block_message,
    "get_open_break": get_open_break,
    "get_user_by_id": get_user_by_id,
    "invalidate_admin_employee_rows_cache": invalidate_admin_employee_rows_cache,
    "is_overbreak": is_overbreak,
    "log_activity": log_activity,
    "login_required": login_required,
    "minutes_to_hm": minutes_to_hm,
    "now_str": now_str,
    "redirect": redirect,
    "request": request,
    "resolve_attendance_time_in_details": resolve_attendance_time_in_details,
    "save_uploaded_file": save_uploaded_file,
    "session": session,
    "today_str": today_str,
...
register_employee_portal_routes(app, {
    "IMAGE_EXTENSIONS": IMAGE_EXTENSIONS,
    "build_break_sessions": build_break_sessions,
    "build_employee_attendance_calendar": build_employee_attendance_calendar,
    "build_employee_history_records": build_employee_history_records,
    ...
    "generate_password_hash": generate_password_hash,
    "get_employee_break_limit": get_employee_break_limit,
    "get_employee_error_record_summary": get_employee_error_record_summary,
    "get_employee_override_status_for_date": get_employee_override_status_for_date,
    "get_employee_released_payroll_runs": get_employee_released_payroll_runs,
    "get_leave_balance_summary": get_leave_balance_summary,
    "get_live_attendance_context": get_live_attendance_context,
    "get_manual_attendance_block_message": get_manual_attendance_block_message,
    "get_open_break": get_open_break,
    "get_open_overtime_session": get_open_overtime_session,
    "get_overbreak_minutes": get_overbreak_minutes,
    "get_user_by_id": get_user_by_id,
    "get_user_live_status": get_user_live_status,
    "log_activity": log_activity,
    "login_required": login_required,
    "make_response": make_response,
    "minutes_to_hm": minutes_to_hm,
    "now_dt": now_dt,
    "parse_calendar_month": parse_calendar_month,
    "redirect": redirect,
    "render_template": render_template,
    "request": request,
    "save_uploaded_file": save_uploaded_file,
...
register_auth_routes(app, {
    ...
    "get_home_route_for_user": get_home_route_for_user,
    "get_user_by_id": get_user_by_id,
...
register_history_routes(app, {
    "BytesIO": BytesIO,
    "Response": Response,
    "build_admin_history_records": build_admin_history_records,
    "flash": flash,
    "get_department_options": get_department_options,
    "login_required": login_required,
    "minutes_to_decimal_hours": minutes_to_decimal_hours,
    "minutes_to_hm": minutes_to_hm,
    "parse_break_limit_minutes": parse_break_limit_minutes,
    "redirect": redirect,
    "render_template": render_template,
    "request": request,
    "today_str": today_str,
...
register_reports_routes(app, {
    ...
    "autosize_workbook_sheet": autosize_workbook_sheet,
    "flash": flash,
    "get_cached_admin_reports_data": get_cached_admin_reports_data,
    "get_department_options": get_department_options,
    "login_required": login_required,
    "minutes_to_hm": minutes_to_hm,
    "normalize_admin_report_filters": normalize_admin_report_filters,
    "redirect": redirect,
    "render_template": render_template,
    "request": request,
    "url_for": url_for,
...
register_payroll_routes(app, {
    "BytesIO": BytesIO,
    "Response": Response,
    "build_effective_payroll_adjustments": build_effective_payroll_adjustments,
    "build_employee_payslip_pdf_bytes": build_employee_payslip_pdf_bytes,
    "build_employee_payslip_pdf_filename": build_employee_payslip_pdf_filename,
    "build_payroll_rows": build_payroll_rows,
    "build_payroll_stats": build_payroll_stats,
    "enrich_admin_payroll_run": enrich_admin_payroll_run,
    "execute_db": execute_db,
    "fetchall": fetchall,
    "fetchone": fetchone,
    "flash": flash,
    ...
    "get_department_options": get_department_options,
    "get_employee_options": get_employee_options,
    "get_employee_released_payroll_item": get_employee_released_payroll_item,
    "get_employee_released_payroll_runs": get_employee_released_payroll_runs,
    "get_payroll_adjustments": get_payroll_adjustments,
    "get_payroll_employee_filter_label": get_payroll_employee_filter_label,
    ...
    "log_activity": log_activity,
    "login_required": login_required,
    "normalize_payroll_filters": normalize_payroll_filters,
    "now_str": now_str,
    "parse_iso_date": parse_iso_date,
    "parse_money_value": parse_money_value,
    "payroll_filter_redirect_args": payroll_filter_redirect_args,
    "quote": quote,
    "redirect": redirect,
    "render_template": render_template,
    "request": request,
...
register_employee_routes(app, {
    "BREAK_LIMIT_MINUTES": BREAK_LIMIT_MINUTES,
    "DEFAULT_BREAK_WINDOW_END": DEFAULT_BREAK_WINDOW_END,
    "DEFAULT_BREAK_WINDOW_START": DEFAULT_BREAK_WINDOW_START,
    "DEFAULT_PAID_LEAVE_DAYS": DEFAULT_PAID_LEAVE_DAYS,
    ...
    "IMAGE_EXTENSIONS": IMAGE_EXTENSIONS,
    "Response": Response,
    "SCHEDULE_SPECIAL_RULE_LABELS": SCHEDULE_SPECIAL_RULE_LABELS,
    "SCHEDULE_SPECIAL_RULE_OPTIONS": SCHEDULE_SPECIAL_RULE_OPTIONS,
    "WEEKDAY_OPTIONS": WEEKDAY_OPTIONS,
    "build_future_schedule_change_map": build_future_schedule_change_map,
    "build_schedule_special_rule_label": build_schedule_special_rule_label,
    "create_notification": create_notification,
    "delete_uploaded_file_if_unused": delete_uploaded_file_if_unused,
    "datetime": datetime,
    "execute_db": execute_db,
    "fetchall": fetchall,
    "fetchone": fetchone,
    "find_employee_identifier_conflict": find_employee_identifier_conflict,
    "flash": flash,
    "generate_code128_svg_markup": generate_code128_svg_markup,
    "generate_password_hash": generate_password_hash,
    "get_future_schedule_changes": get_future_schedule_changes,
    "get_recent_employee_schedule_history": get_recent_employee_schedule_history,
    "get_schedule_day_codes": get_schedule_day_codes,
    "get_schedule_preset": get_schedule_preset,
    "get_schedule_presets": get_schedule_presets,
    "get_schedule_special_dates": get_schedule_special_dates,
    "get_client_ip": get_client_ip,
    "get_db": get_db,
    ...
    "invalidate_admin_employee_rows_cache": invalidate_admin_employee_rows_cache,
    "invalidate_reports_cache": invalidate_reports_cache,
    "invalidate_schedule_change_apply_state": invalidate_schedule_change_apply_state,
    "invalidate_schedule_special_rule_cache": invalidate_schedule_special_rule_cache,
    "log_activity": log_activity,
    "login_required": login_required,
    "normalize_schedule_days": normalize_schedule_days,
    "normalize_schedule_special_rule_type": normalize_schedule_special_rule_type,
    "now_dt": now_dt,
    "now_str": now_str,
    "parse_break_limit_minutes": parse_break_limit_minutes,
    "parse_iso_date": parse_iso_date,
    "parse_money_value": parse_money_value,
    "parse_non_negative_int": parse_non_negative_int,
    "parse_shift_end": parse_shift_end,
    "parse_shift_start": parse_shift_start,
    "queue_future_schedule_change": queue_future_schedule_change,
    "record_employee_schedule_history": record_employee_schedule_history,
    "redirect": redirect,
    "render_template": render_template,
    "request": request,
    "resolve_schedule_assignment": resolve_schedule_assignment,
    "save_uploaded_file": save_uploaded_file,
    "schedule_preset_matches_department": schedule_preset_matches_department,
    "secure_filename": secure_filename,
    "session": session,
    "summarize_employee_admin_changes": summarize_employee_admin_changes,
    "timedelta": timedelta,
    "today_str": today_str,
    "uploaded_file_exists": uploaded_file_exists,
    "url_for": url_for,
